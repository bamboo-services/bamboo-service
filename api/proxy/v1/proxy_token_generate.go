...
type ProxyTokenGenerateReq struct {
	...
	Name        string      `json:"name" v:"required#请输入令牌名称"`
	Description string      `json:"description" v:"length:0,255#令牌描述长度不能超过255个字符"`
	ExpiredAt   *gtime.Time `json:"expired_at" v:"required|regex:^\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}$#请输入令牌过期时间|令牌过期时间格式不正确"`
}
