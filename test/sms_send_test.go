...
func TestSmsSend(t *testing.T) {
	if false {
		...
		// 读取文件
		getJson := gjson.New(gfile.GetContents("../aliyun.json"))
		constant.AliyunSmsSignName = "锋楪"
		constant.AliyunSmsCodeTemplateCode = "SMS_468930484"
		constant.AliyunAccessKey = getJson.Get("Key.AccessKeyID").String()
		constant.AliyunSecretKey = getJson.Get("Key.AccessKeySecret").String()
